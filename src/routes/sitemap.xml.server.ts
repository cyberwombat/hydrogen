...
function shopSitemap(data: SitemapQueryData, baseUrl: string) {
  const productsData = flattenConnection<Product>(data.products)
  ...
  const collectionsData = flattenConnection<Collection>(data.collections)
  ...
  const pagesData = flattenConnection<Page>(data.pages)
